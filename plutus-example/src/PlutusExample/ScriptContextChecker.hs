...
data ScriptContextError = NoScriptsInByronEra
                        ...
                        | PlutusV2TranslationError Alonzo.TranslationError
                        deriving Show
...
createAnyCustomRedeemer pScriptVer sbe pparams utxo eInfo sStart (ShelleyTx ShelleyBasedEraBabbage ledgerTx) = do
  let txBody = getField @"body" ledgerTx
      ...
      _certs = seqToList $ getField @"certs" txBody
      vldt = getField @"vldt" txBody
      _reqSigners = Set.toList $ getField @"reqSignerHashes" txBody
      Alonzo.TxDats _datumHashMap = getField @"txdats" $ getField @"wits" ledgerTx
  ...
  case pScriptVer of
    ...
    PlutusScriptV2 -> do
      _bV2Ins <- first PlutusV2TranslationError $ Prelude.mapM (Babbage.txInfoInV2 bUtxo) txins
      _bV2RefIns <- first PlutusV2TranslationError $ Prelude.mapM (Babbage.txInfoInV2 bUtxo) refTxins
      -- TODO: Tx outs appear to not be working
      _bV2Outputs <- first PlutusV2TranslationError $ zipWithM Babbage.txInfoOutV2 (repeat Babbage.OutputFromInput) outputs
      -- TODO: fee won't work with build command. Double check this. Instead build the dummy
      -- tx with build raw to get around this!
      let _bV2fee = V2.singleton V2.adaSymbol V2.adaToken fee
      -- TODO: Also not working and I don't know why
      valRange <-
        first IntervalConvError
      ...
      Right . AnyPV2CustomRedeemer
        $ PV2CustomRedeemer
            ...
            , pv2Wdrl = PMap.empty
            , pv2ValidRange = valRange
            , pv2Signatories = [] --  Prelude.map Alonzo.transKeyHash reqSigners
            , pv2Redeemers = PMap.empty
            , pv2Data = PMap.empty -- PMap.fromList . Prelude.map Alonzo.transDataPair $ Map.toList datumHashMap
            }
